...
class Model(nn.Module):
    def __init__(self, n_stride=128, n_channel=4):
        ...
        self.layers = nn.Sequential(
            QInvertedResidual(     3, base*1, 2, 2), 
            QInvertedResidual(base*1, base*2, 2, 2), 
    ...
    def forward(self, x, device):
        ...
        out_time = torch.sigmoid(out_time) * self.n_stride 
...
class CTLoss(nn.Module):
    ...
    def __init__(self):
        ...
        # 加权
        weight = torch.Tensor([1, 1.5, 1.5]).float()
        self.ce = nn.CrossEntropyLoss(weight=weight, reduction="sum", ignore_index=-1)
...
def main(args):
    stride = 32
    nchannel = 4
    model_name = f"ckpt/new.{stride}-{nchannel}.wave"
    data_tool = utils.Data(batch_size=32, n_thread=1, strides=stride, n_length=3072)
