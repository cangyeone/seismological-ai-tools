    ...
    def process_multithread(self, in_queue, out_queue):
        ...
        while True:
            ...
            if pt == -100 and st == -100:
                ...
                logit = -np.ones([1, n_legnth_s, 2]) 
                if p_idx_s - 1>0:
                    logit[0, p_idx_s-1:p_idx_s+2, 0] = -2 
                if s_idx_s - 1>0:
                    logit[0, s_idx_s-1:s_idx_s+2, 0] = -2                  
    ...
    def process_multithread(self, in_queue, out_queue):
        ...
        while True:
            ...
            if pt == -100 and st == -100:
                ...
                p_idx = int(pt)
                s_idx = int(st)
                begin = np.random.randint(0, max(p_idx//2, 2)) 
                ...
                logit = -np.ones([1, n_legnth_s, 2]) 
                if p_idx_s-2>0:
                    logit[0, p_idx_s-2:p_idx_s+3, 0] = -2  
                if s_idx_s-2>0:
                    logit[0, s_idx_s-2:s_idx_s+3, 0] = -2  
                if p_idx_s > 0:
                    logit[0, p_idx_s:p_idx_s+1, 0] = 0 
                    logit[0, p_idx_s:p_idx_s+1, 1] = p_idx % n_stride  
                if s_idx_s > 0:
                    logit[0, s_idx_s:s_idx_s+1, 0] = 1  
                    logit[0, s_idx_s:s_idx_s+1, 1] = s_idx % n_stride  
                wave = np.reshape(wave, [1, -1, 3])
                B, T, C = wave.shape 
                data = np.zeros([1, self.n_length, 3]) 
                data[:, :T-begin, :] = wave[:, begin:, :]
                wave = data
    ...
    def process_multithread(self, in_queue, out_queue):
        ...
        while True:
            ...
            if pt == -100 and st == -100:
                ...
                s_idx = s_idx - begin 
                data = np.zeros([1, self.n_length, 3])
                if self.n_length > 6000:
                    data[:, :6000, ] = np.reshape(wave, [1, -1, 3])
                wave = data 
            if p_idx < 0:
                isnoise = True 
            else:
