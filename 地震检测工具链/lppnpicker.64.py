...
class ConvTBNReLU(nn.Sequential):
    ...
    数值范围，从而有利于量化处理。
    """
    def __init__(self, n_in, n_out, stride=(1, 2)):
        # padding为same时两边添加(K-1)/2个0
        # 本层构建三个层，即0：卷积，1：批标准化，2：ReLU
        super(ConvTBNReLU, self).__init__(
            nn.UpsamplingBilinear2d(scale_factor=tuple(stride)), 
            QInvertedResidual(n_in, n_out, 1, 3), 
        )
class Model(nn.Module):
    def __init__(self, n_stride=8):
        super().__init__()
        self.n_stride = 64 # 总步长 
        base = 16 
        self.layers = nn.Sequential(
            ConvBNReLU(3, base*1, groups=1), 
            QInvertedResidual(base*1, base*1, 2, 3), 
            QInvertedResidual(base*1, base*1, 1, 3), 
            QInvertedResidual(base*1, base*2, 2, 3), 
            QInvertedResidual(base*2, base*2, 1, 3), 
            QInvertedResidual(base*2, base*3, 2, 3),
            QInvertedResidual(base*3, base*3, 1, 3), 
            QInvertedResidual(base*3, base*4, 2, 3), 
            QInvertedResidual(base*4, base*4, 1, 3), 
            QInvertedResidual(base*4, base*5, 2, 3), 
            QInvertedResidual(base*5, base*5, 1, 3), 
            QInvertedResidual(base*5, base*6, 2, 3), 
            QInvertedResidual(base*6, base*6, 1, 3)       
        )
        self.class_encoder = nn.Sequential(
            QInvertedResidual(base*6, base*7, 2, 3), 
            QInvertedResidual(base*7, base*8, 2, 3), 
            QInvertedResidual(base*8, base*9, 2, 3), 
            QInvertedResidual(base*9, base*10, 2, 3), 
            QInvertedResidual(base*10, base*11, 2, 3), #SNR added 
            ConvTBNReLU(base*11, base*10),
            ConvTBNReLU(base*10, base*9),
            ConvTBNReLU(base*9, base*8), 
            ConvTBNReLU(base*8, base*7),  
            ConvTBNReLU(base*7, base*6), 
        )
        self.cl = nn.Conv2d(base * 6 * 2, 5, 1) 
        self.tm = nn.Conv2d(base * 6 * 2, 1, 1)
    ...
    def forward(self, x, device):
        with torch.no_grad():
            ...
            max2, max2idx = torch.max(max1, dim=1, keepdim=True) 
            wave /= (max2 + 1e-6)  
            wave = wave.unsqueeze(2)
            #print("数据维度", wave.shape)
            x1 = self.layers(wave)
            x2 = self.class_encoder(x1) 
            #print("数据维度", wave.shape, x1.shape, x2.shape)
            ...
            for itr in range(4):
                ...
                while True:
                    ...
                    select[idx] = 1 
                    count += 1 
                    selidx = torch.masked_select(selidx, torch.abs(ref-ntime)>global_parameter.nmslen)
                    nprob = torch.masked_select(nprob, torch.abs(ref-ntime)>global_parameter.nmslen)
                    ntime = torch.masked_select(ntime, torch.abs(ref-ntime)>global_parameter.nmslen)
...
class Process():
    def __init__(self, infile="data", outfile="data/out", model="", device="cpu:0"):
        ...
        for itr in range(n_thread):
            t_data = multiprocessing.Process(target=self.process, args=(feedq, dataq)) 
            t_data.start() 
        if os.path.isdir(infile):# 检查是否是文件，如果是目录则遍历目录找mseed
            multiprocessing.Process(target=self.feed2, args=(feedq, self.processed_dict)).start()
        else:# 如果是文件则读取文件
            multiprocessing.Process(target=self.feed3, args=(feedq, self.processed_dict)).start()
        for itr in range(global_parameter.npicker):
            multiprocessing.Process(target=self.infer, args=(dataq, outq)).start()
    ...
    def process(self, feedq, dataq):
        ...
        while True:
            ...
            for fdir in file_dir:
                st = obspy.read(os.path.join(base_dir, fdir)).merge(fill_value=0)
                st.trim(pad=True, nearest_sample=True, fill_value=0)
                file_data = st[0]
                #print("起始时间")
                stime = datetime.datetime.strptime(f"{file_data.stats.starttime}", r"%Y-%m-%dT%H:%M:%S.%fZ")
                wave = file_data.data 
                waves.append(wave) 
                btime.append(stime) 
            ...
            fdata = bandpass(wave, global_parameter.bandpass[0], global_parameter.bandpass[1], 100)
            if is100Hz==False:
                print("数据不是100Hz", file_dir)
                continue 
            outinfo = {"root":base_dir, "key":file_dir[0], "data":datas, "stime":reftime, "fdata":fdata, "pkey":temp["pkey"]} 
            dataq.put(outinfo)
    ...
    def feed2(self, feedq, processed_dict):
        if True: 
            ...
            for root, dirs, files in os.walk(self.infile):
                ...
                file_dict = {}
                for name in files:
                    if name.endswith(global_parameter.filenametag)==False:continue 
                    sname = name.split(".") 
                    #if sname[0] not in ["YN", "GX", "GZ", "SC", "XZ"]:continue
                    key = ".".join([sname[i] for i in global_parameter.namekeyindex])
                    if key in file_dict:
                ...
                for key in file_dict:
                    ...
                    name = key.split(".")[0] 
                    if processed_key in processed_dict:continue 
                    three1 = []
                    three2 = []
                    for fn in file_dict[key]:
                        #print(fn)
                        sfn = fn.split(".")[global_parameter.channelindex]  #######################################################################需要标注BHE或SHE,N,Z
                    ...
                    if len(three1)==global_parameter.nchannel:
                        ...
                        continue 
                    info = {"root":root,"key":key, "dirs":three, "pkey":processed_key}
                    #print(info)
    ...
    def infer(self, dataq, outq):
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                outq.put([root, nnout, temp["data"], temp["stime"], temp["fdata"], temp["pkey"]])
    def write(self, outq):
        # 输出文件
        import scipy.signal as signal 
        ...
        while True:
            ...
            for itr in range(6):# 六个震相
                ...
                for p, t in zip(phases[:, 2], phases[:, 1]):
                    ...
                    if "P" in phase_dict[itr]:
                        ...
                        pki = peaks[np.argsort(np.abs(peaks-global_parameter.snritv))]  
                        pkv = segs[pki]
                    else:
                        b = np.max([0, pidx-global_parameter.snritv])
                        pre = fdata[b:pidx]
                    ...
                    if len(w)>0:
                        ...
                        amp = -1
                    ptime = stime + datetime.timedelta(seconds=t/global_parameter.samplerate) 
                    if t/100<0:
                        print(t, ptime)
                    if len(pki)>=2:
                        files.write(f"{phase_dict[itr]},{t/global_parameter.samplerate:.3f},{p:.3f},{ptime.strftime('%Y-%m-%d %H:%M:%S.%f')},{snr:.3f},{amp},{sta},{b1},{e1},{b2},{e2},{b3},{e3},{pki[0]},{pkv[0]},{pki[1]},{pkv[1]}\n") 
                    elif len(pki)==1:
                        files.write(f"{phase_dict[itr]},{t/global_parameter.samplerate:.3f},{p:.3f},{ptime.strftime('%Y-%m-%d %H:%M:%S.%f')},{snr:.3f},{amp},{sta},{b1},{e1},{b2},{e2},{b3},{e3},{0},{0},{0},{0}\n") 
                    elif len(pki)==0:
                        files.write(f"{phase_dict[itr]},{t/global_parameter.samplerate:.3f},{p:.3f},{ptime.strftime('%Y-%m-%d %H:%M:%S.%f')},{snr:.3f},{amp},{sta},{b1},{e1},{b2},{e2},{b3},{e3},{0},{0},{0},{0}\n") 
                    if pname != "N" and global_parameter.ifreal:
                        # 制作REAL日期文件夹
                        timedir = ptime.strftime("%Y%m%d") 
                        baseptime = datetime.datetime.strptime(timedir, "%Y%m%d")
                        ...
                        file_ = open(os.path.join(basedir, file_name), "a") # 关联文件名
                        # 52231.555 0.986 0
                        file_.write(f"{(ptime-baseptime).total_seconds()} {p:.3f} {0}\n")
                        file_.flush() 
                        file_.close()
                #if pname != "N":
                    #file_.close()
            ...
            acc_index += 1 
            print(f"已处理:{acc_index}, 当前完成{root}, {stime}, 平均用时{np.mean(acc_time)}")
# nohup python picktest20.py -o result/2020.1107.200km -m ckpt/new.200km.wave -d cuda:0 > result/2020.200km.log 2>&1 &
# 873412
import argparse
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--output', default="odata/3days.new.s64", help="输出文件名")      
    parser.add_argument('-m', '--model', default="ckpt/lppn64.model", help="模型文件lppnmodel")       
    parser.add_argument('-d', '--device', default="cuda:0", help="模型文件lppnmodel")                                                      
