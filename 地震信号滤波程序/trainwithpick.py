...
import os 
import numpy as np 
from pickmodel import Model as PickModel 
from pickmodel import CTLoss as PickLoss 
...
def main(args):
    ...
    dist = args.dist 
    model_name = f"ckpt/denoise.wave.with.pick"
    data_tool = utils.DataWithPick(batch_size=32, n_thread=1, strides=8, n_length=3072)
    ...
    pickmodel = PickModel(8, 8)
    pickmodel.eval() 
    pickmodel.to(device) 
    pickmodel.load_state_dict(torch.load("ckpt/8-8.wave", map_location=device))
    lossfn2 = PickLoss()
    lossfn2.to(device)
    ...
    for step in range(100000000):
        ...
        #print(wave.shape) 
        clean2 = model(wavenoise)
        clean = clean2.permute(0, 2, 1)
        loss = ((clean - wave) ** 2).sum() 
        loss.backward(retain_graph=True)

        dc = torch.tensor(a3[:, :, 0], dtype=torch.long).to(device) 
        dt = torch.tensor(a3[:, :, 1], dtype=torch.float).to(device)
        oc, ot = pickmodel(clean2, device)
        loss2 = lossfn2(oc, ot, dc, dt) 
        loss2.backward()   
        pickmodel.zero_grad()
        ...
        if step % 20 == 0:
            ...
            ax.plot(t, w, alpha=0.3, c="k", label="噪声+波形")
            ax.set_ylim((-1.2, 1.2))
            print(pt, st)
            #ax.axvline(pt, c="r", label="P") 
            #ax.axvline(st, c="b", label="S")
            ax.legend(loc="upper right")
            ax = fig.add_subplot(gs[1, 0]) 
            t = np.arange(len(d)) * 0.01
            ax.plot(t, d, c="k", alpha=0.5, label="波形") 
            ax.set_ylim((-1.2, 1.2))
            #ax.axvline(pt, c="r", label="P") 
            #ax.axvline(st, c="b", label="S")
            ax.legend(loc="upper right")
            ...
            #ax.plot(w-d+1, c="k", alpha=0.5, label="噪声") 
            #ax.plot(w-c-1, c="k", alpha=0.5, label="波形-滤波后") 
            w = bandpass(nz, 1, 10, 100)
            w -= np.mean(w) 
            w /= (np.max(np.abs(w))+1e-6)
            t = np.arange(len(w)) * 0.01
            ax.plot(t, w, c="k", alpha=0.5, label="带通滤波(1-10Hz)")
            ax.set_ylim((-1.2, 1.2))
            #ax.axvline(pt, c="r", label="P") 
            #ax.axvline(st, c="b", label="S")
            ax.legend(loc="upper right") 
            ...
            t = np.arange(len(c)) * 0.01
            c -= np.mean(c) 
            c /= (np.max(np.abs(c))+1e-6)
            ax.plot(t, c, c="k", alpha=0.5, label="CNN滤波") 
            ax.set_ylim((-1.2, 1.2))
            #ax.axvline(pt, c="r", label="P") 
            #ax.axvline(st, c="b", label="S")
            ax.legend(loc="upper right")
