...
class Model(nn.Module):
    def __init__(self, n_stride=8):
        ...
        self.layers = nn.Sequential(
            ConvBNReLU(3, F*1),
            QInvertedResidual(F*1, F*2, 2, 2),
            QInvertedResidual(F*2, F*2, 1, 2),
            QInvertedResidual(F*2, F*3, 2, 2),
            QInvertedResidual(F*3, F*3, 1, 2),
            QInvertedResidual(F*3, F*4, 2, 2),
            QInvertedResidual(F*4, F*4, 1, 2)
        )
        self.class_encoder = nn.Sequential(
            QInvertedResidual(F*4, F*4, 2, 2),
            QInvertedResidual(F*4, F*4, 2, 2),
            QInvertedResidual(F*4, F*4, 2, 2),
            ConvTBNReLU(F*4, F*4, [1, 5], stride=[1, 2], padding=[
        ...
        self.cl = nn.Conv2d(F * 4 * 2, 3, 1)
        self.tm = nn.Conv2d(F * 4 * 2, 1, 1)
