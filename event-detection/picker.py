            ...
            for root, dirs, files in os.walk(self.infile):
                ...
                for key in file_dict:
                    ...
                    if processed_key in processed_dict:
                        continue
                    threes = [[] for i in range(len(global_parameter.chnames))]
                    #three2 = []
                    ...
                    for three in threes:
                        if len(three)==global_parameter.nchannel:
        ...
        while True:
            ...
            for itr in range(6):  # 六个震相
                ...
                for p, t in zip(phases[:, 2], phases[:, 1]):
                    ...
                    if len(pki) >= 2:
                        files.write(
                            f"{phase_dict[itr]},{t/global_parameter.samplerate:.3f},{p:.3f},{ptime.strftime('%Y-%m-%d %H:%M:%S.%f')},{snr:.3f},{amp},{sta}\n")
                    elif len(pki) == 1:
                        files.write(
                            f"{phase_dict[itr]},{t/global_parameter.samplerate:.3f},{p:.3f},{ptime.strftime('%Y-%m-%d %H:%M:%S.%f')},{snr:.3f},{amp},{sta}\n")
                    elif len(pki) == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-o', '--output', default="odata/testonnx", help="输出文件名")
    parser.add_argument(
        '-m', '--model', default="pickers/rnn.onnx", help="模型文件lppnmodel")
