...
class Process():
    def __init__(self, infile="data", outfile="data/out", model="", device="cpu:0"):
        ...
        for itr in range(n_thread):
            if global_parameter.is_seed:
                t_data = multiprocessing.Process(
                target=self.process4, args=(feedq, dataq, itr))
            else:
                t_data = multiprocessing.Process(
        ...
        if os.path.isdir(infile):  # 检查是否是文件，如果是目录则遍历目录找mseed
            if global_parameter.is_seed:
                feedt = multiprocessing.Process(target=self.feed4, args=(
        ...
        while True:
            if dataq.empty() and feedq.empty() and outq.empty():
                ...
                time.sleep(0.1)
    def process4(self, feedq, dataq, fid):
        ...
        while True:
            ...
            #print("测试", file_dir)
            if len(threes) != global_parameter.nchannel:
            ...
            maxlen = 0 
            if True:
                for tr, name, pkey, path in threes:
                    file_data = tr
                    # print("起始时间")
            ...
            if is100Hz == False:
                ...
                continue
            outinfo = {"root": path, "key": pkey, "data": datas,
                       "stime": reftime, "fdata": fdata, "pkey": pkey, "errinfo":errinfo}
            dataq.put(outinfo)
    ...
    def feed4(self, feedq, processed_dict):
        if True:
            # 其他文件夹
            print("遍历目录为", self.infile)
            feedinfos = []
            for root, dirs, files in os.walk(self.infile):
                ...
                file_dict = {}
                for name in files:
                    if name.endswith(global_parameter.filenametag) == False:#只处理seed文件
                        continue
                    processed_key = f"{root}/{name}"
                    path = os.path.join(root, name)
                    stream = obspy.read(path).merge(fill_value=0)
                    stream.resample(global_parameter.samplerate)
                    stream.trim(pad=True, nearest_sample=True, fill_value=0)
                    ...
                    for trace in stream:
                        ...
                        processed_key = f"{root}/{name}/{key}" 
                        #print(processed_key)
                        newpath = f"{path}/{key}"
                        if processed_key in processed_dict:
                            continue
                        skey = f"{net}.{sta}.{loc}"
                        file_list.append([trace, cha, processed_key])
                        if skey in file_dict:
                            file_dict[skey].append([trace, cha, processed_key])
                        else:
                            file_dict[skey] = [[trace, cha, processed_key]]
                    
                    allthrees = []
                    for skey, files in file_dict.items():
                        #print(skey, len(files))
                        threes = [[0 for i in range(global_parameter.nchannel)] for i in range(len(global_parameter.chnames))]
                        for trace, sfn, pkey in files:
                            for cid, chn in enumerate(global_parameter.chnames):
                                sid, = np.where(np.array(chn)==sfn)
                                if len(sid)==0:continue 
                                threes[cid][sid[0]] = [trace, sfn, pkey, newpath] 
                        allthrees.append(threes)
                    for threes in allthrees:
                        for three in threes:
                            if 0 not in three:
                                trace, sfn, pkey, path = three[-1]
                                processed_dict[pkey] = 0
                                feedq.put(three)
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                if len(temp)==1:
                    ...
                    continue 
                if global_parameter.is_seed==False:
...
if __name__ == "__main__":
    # 处理输出某个时间之后的所有震相
    parser = argparse.ArgumentParser(description="拾取波形数据")
    parser.add_argument('-i', '--input', default="data/seed6", help="拾取波形文件夹位置")
    parser.add_argument(
        '-o', '--output', default="data/seed6.phase", help="输出文件名，不带扩展名")
    parser.add_argument(
        '-m', '--model', default="pickers/rnn.origdiff.pnsn.jit", help="模型文件")
