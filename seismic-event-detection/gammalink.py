...
class DataLPPN():
    def __init__(self, infilename, stationfile):
        ...
        for line in file_.readlines():
            ...
            key = ".".join(sline[2:4])
            x, y, z = float(sline[0]), float(sline[1]), float(sline[5])
    ...
    def skipdata(self):
        ...
        self.start += gpars.win_stirde 
        return 0       
def associationt(picks, data_tool):
    vel = gpars.vel 
...
def main(args):
    ...
    file_ = open(args.output, "w", encoding="utf-8")
    ...
    for t in range(L):
        ...
        events = associationt(data, data_tool)
        print(f"处理第{t}天的数据...")
        for event in events:
            for eve in event:
                if "time" in eve:
                    ...
                    tstr = eve["time"]
                    file_.write(f"{tstr},{lon:.4f},{lat:.4f},{z:.3f},{x:3f},{y:.3f}\n")
        file_log.write(f"{t}\n")
import argparse
if __name__ == "__main__":
    # 处理输出某个时间之后的所有震相
    parser = argparse.ArgumentParser(description="拾取连续波形")          
    parser.add_argument('-i', '--input', default="odata/luding.rnn.01.pick.901_1022.txt.tat", help="拾取文件")       
    parser.add_argument('-o', '--output', default="odata/luding.901-1022.gamma.txt", help="输出文件名")   
    parser.add_argument('-s', '--station', default="odata/station.loc", help="输出文件名")     
